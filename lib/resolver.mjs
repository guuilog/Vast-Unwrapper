...
export async function mergeWrapperImpressionsIntoInlineXml(inlineAdmXml, wrapperUrl, { debug = false } = {}) {
  ...
  const pre = {
    inlineImps: normalizeImpressions(getInline(inlineDoc)?.Impression).length
  };

  const mergedDoc = mergeWrapperIntoInline(wrapperDoc, inlineDoc);

  const post = {
    mergedImps: normalizeImpressions(getInline(mergedDoc)?.Impression).length
  };
  ...
  return builder.build(mergedDoc);
}
