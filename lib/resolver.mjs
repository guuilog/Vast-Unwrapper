...
function mergeWrapperIntoInline(wrapperDoc, inlineDoc) {
  ...
  // Impressions
  const wImp = toArr(adW.Impression?.[0]?.["#text"] ? adW.Impression : adW.Impression || []);
  const iImp = toArr(adI.Impression || []);
  const mergedImp = mergeUnique(iImp, wImp, x => (typeof x === "string" ? x : x?.["#text"] || JSON.stringify(x)));
  if (mergedImp.length) adI.Impression = mergedImp;
  ...
  const mergedViewable = mergeUnique(
    toArr(iVI.Viewable || []), toArr(wVI.Viewable || []), v => (typeof v === "string" ? v : v?.["#text"] || JSON.stringify(v))
  );
  const mergedUndet = mergeUnique(
    toArr(iVI.ViewUndetermined || []), toArr(wVI.ViewUndetermined || []), v => (typeof v === "string" ? v : v?.["#text"] || JSON.stringify(v))
  );
  const vi = {};
